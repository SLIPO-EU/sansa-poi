...
object poiClustering {
  ...
  def getCategoryValues(sparkSession: SparkSession, data: RDD[Triple]): RDD[(Long, Categories)] = {
    ...
    val categoriesIdValues = categoryIds.map(x => (
      x.getSubject.toString().replace(termPrefix, "").toLong,
  ...
  def main(args: Array[String]) {
    ...
    val poiCoordinates = dataRDD.filter(x => x.getPredicate.toString().equalsIgnoreCase(coordinatesPredicate))
      .map(x => (
        x.getSubject.toString().replace(poiPrefix, "").replace("/geometry", "").toLong,
    ...
    val poiRawCategoriesVienna = poiCategoriesVienna.map(x => (
      x.getSubject.toString().replace(poiPrefix, "").toLong,
